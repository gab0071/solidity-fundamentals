...
contract voting_system {
    ...
    function candidateRegistration(
        ...
        require(
            now <= (start_voting + 5 minutes),
            'Candidates can no longer be submitted'
        );
        bytes32 candidate_hash = keccak256(
            abi.encodePacked(_candidateName, _candidateAge, _candidateId)
        );
    ...
    function Vote(string memory _candidate) public {
        ...
        for (uint256 j = 0; j < candidates.length; j++) {
            ...
            if (
                keccak256(abi.encodePacked(candidates[j])) ==
